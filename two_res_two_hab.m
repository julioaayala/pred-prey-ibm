function two_res_two_hab(varargin)
    
    %Parameters
    t_end = 200;
    F = 2; % Fecundity rate
    K = 500; % Carrying capacity
    P_d = 0.1; % Probability of dispersal
    a_0 = 2; % Base attack rate
    sigma_alpha = 1; % Resource niche width (Unused
    ...
    for i=1:num_populations
        ...
        population(i) = Individual(1, i, a_0, i, sigma_alpha, sigma_beta);
    end
    ...
    % Initialize plot
    figure;
    legendStr = union("Population in habitat " + string(1:num_populations), ...
                      "Resource in habitat " + string(1:num_habitats));
    
    colors = ["#0072BD", "#D95319", "#EDB120", "#7E2F8E", "#77AC30", "#4DBEEE", "#A2142F"];
    for i=1:(num_populations+num_habitats)
    ...
    for t = 1:t_end
        ...
        pop_habitats = [population.habitat];
        % Count individuals by habitat
        for i = 1:length(pop_habitats)
            tmp_popsize(pop_habitats(i)) = tmp_popsize(pop_habitats(i)) + 1;
        end
        for i = 1:length(population)
            % Reproduction
            for j=1:F % Create for every offspring
                ...
                offspring.fitness = offspring.calc_fitness_beta(resources(currhab,:), tmp_popsize(currhab), F);
        ...
        for p=1:num_habitats
            addpoints(h(p+num_populations),t,resource_abundance(t,p));
        end
